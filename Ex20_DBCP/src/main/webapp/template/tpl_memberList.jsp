...
<%
	// 컨텍스트 경로 추출
	String ctxPath=request.getContextPath(); // => /Ex20_DBCP
	String ctxName=ctxPath.substring(1); // => Ex20_DBCP
	out.println("컨텍스트 경로 : "+ctxPath+"<br>");
	out.println("컨텍스트 이름 : "+ctxName+"<br>");
%>
...
<body>
	...
	<c:if test="${!dataList.isEmpty()}">
		...
		<c:forEach var="member"  items="${dataList}">
			<ul>
				...
				${member}
				&nbsp;
				<button onclick="location.href='<%=ctxPath%>/memberUpdateForm.do?id=${member.getId()}';">Update</button>
				<button onclick="location.href='<%=ctxPath%>/memberDelete.do?id=${member.getId()}';">Del</button>
				</li>
			</ul>
		</c:forEach>
	...
	<div style="text-align:center;">
		<button onclick="location.href='<%=ctxPath%>/memberInsertForm.do';">입력 페이지 이동</button>
	</div>
	...
	<div style="text-align: center;">
		...
		<a href="<%=ctxPath %>/memberList.do?page=1">◀◀ First</a>
		&nbsp;&nbsp;&nbsp;
		<c:choose>
			<c:when test="${groupNumber!=1 }">
				<a href="<%=ctxPath %>/memberList.do?page=${startPage-pageGroup }">◀ Prev</a>
			</c:when>
			...
			<c:forEach var="i" begin="${startPage }" end="${endPage}" >
				...
				<c:choose>
						<c:when test="${i!=cPage}">
						<a href="<%=ctxPath %>/memberList.do?page=${i}">&nbsp; ${i} &nbsp;</a>					
			...
			<c:choose>
				<c:when test="${totalPage!=endPage}">
					<a href="<%=ctxPath %>/memberList.do?page=${startPage+pageGroup}">Next ▶</a>
				</c:when>
			<c:otherwise>
			...
			&nbsp;&nbsp;&nbsp;
			<a href="<%=ctxPath %>/memberList.do?page=${totalPage}">Last ▶▶</a>
